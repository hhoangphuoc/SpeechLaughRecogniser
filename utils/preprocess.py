...
#         batch = switchboard_to_batch(data_name=data_name)
...
def combine_data(
    ...
    try: 
        ...
        # transfrom the word "[laughter]" to "[LAUGHTER]" in transcript column
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--do_process", type=bool, default=True, help="Determine to do the processing steps for all datasets or not, else skip the processing step")
    parser.add_argument("--data_name", type=str, default="switchboard", help="Name of the dataset to process")
...
#-------------------------------------------------------------------------------------------------------------
    ...
    # if args.do_process:
        #Do the processing steps, else skip it
    
    if args.data_name == "switchboard":
        batch = switchboard_to_df(
            data_name = args.data_name,
            csv_dir = args.csv_dir,
            to_csv = args.to_csv
        )

    elif args.data_name == "vocalsound":
        vocalsound_dataset = load_dataset("flozi00/VocalSound_audio_16k", split="train", download_mode='force_redownload')
        batch = vocalsound_to_df(
            data_name = args.data_name,
            batch_audio=[], 
            batch_transcript=[],
            csv_dir = args.csv_dir,
...
#             batch_transcript=[],
        ...
        raise ValueError(f"Dataset {data_name} is not supported")
    # else:
    #     print("NOT PROCESSING ANY DATASET")
    
    # After that, combined the dataset
    if args.do_combine:
        combine_data(
            csv_dir=args.csv_dir,
        ...
        print("No such data to process or combined")
